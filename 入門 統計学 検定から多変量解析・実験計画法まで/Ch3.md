...
- 標本誤差分散
    - 何に使うのか正直分からん(TODO 定義は要確認)
- 母誤差分散
    - (どういうわけか)母分散が既知の場合はこちらを使う。
    - 意味としては、当該母集団から抽出した標本の標本平均の分散は、母分散と標本サイズで決まる。
...
- 標本標準誤差
    - なにに使うのか、正直分からん(TODO 定義は要確認)
- 母標準誤差
    - (どういうわけか)母標準偏差が既知の場合はこちらを使う。
...
母数(母分散等)が既知の場合というのは有り得ないので、そういったケースの時に、標本から母数を推定する場合に使うとよいよ。

不偏推定量を用いた、「不偏誤差分散」「不偏標準誤差」は、下記の用になる。

![不偏誤差分散と不偏標準誤差](./img/19.png)

- 不偏誤差分散
    - 母分散が分からない場合に、標本から母数(母誤差分散)を推定したい場合に使うといいよ(多分)
- 不偏標準誤差
    - 母分散が分からない場合に、標本から母数(母標準誤差)を推定したい場合に使うといいよ(多分)
